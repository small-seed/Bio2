class DirectedGraph:
        ...
        if degree_kind == "in" or degree_kind == "total":
            for node in self.adjacency:
                for neigh in self.adjacency[node]:
                    if degree_kind == "in" or neigh not in self.adjacency[neigh]:
                        degrees[neigh] += 1
        return degrees
    ...
    def bfs_reachable(self, start):
        ...
        while queue:
            ...
            for neigh in self.adjacency[current]:
                if neigh not in visited and neigh not in queue and neigh != current:
                    queue.append(neigh)
        return visited
    ...
    def reachable_distances(self, start):
        ...
        while queue:
            ...
            for neigh in self.adjacency[current]:
                if not any(n == neigh for n, _ in queue + result):
